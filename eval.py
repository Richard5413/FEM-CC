...
def top_k_corrects(preds: torch.Tensor, labels: torch.Tensor, tops: list = [1, 3]):
    """
...
def evaluate(args, model, test_loader):
    ...
    with torch.no_grad():
        ...
        """ t-SNE Visualization """
        # features_array = np.concatenate(features_list, axis=0)  # Combine all features
        # labels_array = np.concatenate(labels_list, axis=0)      # Combine all labels
        #  # Flatten 3D features to 2D
        # if len(features_array.shape) > 2:  # If the array is 3D
        #     features_array = features_array.reshape(features_array.shape[0], -1)  # Flatten the last dimensions

        #  # Select 20 classes randomly
        # unique_classes = np.unique(labels_array)
        # selected_classes = np.random.choice(unique_classes, size=20, replace=False)  # Randomly select 20 classes

        # # Filter features and labels based on selected classes
        # selected_indices = np.isin(labels_array, selected_classes)
        ...
        # scatter = plt.scatter(tsne_result[:, 0], tsne_result[:, 1], c=labels_array, cmap='tab20', alpha=0.6)
        # plt.colorbar(scatter, label='Class')
        # plt.title('t-SNE Visualization of 20 Selected Classes')
        # plt.xlabel('t-SNE Dimension 1')
        # plt.ylabel('t-SNE Dimension 2')

        # # Ensure the directory exists
        # tsne_dir = "/data1/wdh/FEM-CC-ALL/FEM-CC/records/CUB200-Resnetbase/T3000_Test/backup"
        # os.makedirs(tsne_dir, exist_ok=True)

        # # Save the visualization to the file path
        # tsne_save_path = os.path.join(tsne_dir, "tsne_visualization_20_classes.jpg")
        ...
        for name in corrects:
            ...
            if "top-1" in name or "highest-1" in name:
                if acc >= best_top1:
                    best_top1 = acc
                    best_top1_name = name
